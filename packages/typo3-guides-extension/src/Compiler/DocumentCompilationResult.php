...
final class DocumentCompilationResult
{
    ...
    public static function extractFromProjectNode(ProjectNode $projectNode): self
    {
        ...
        // Extract citation targets (need reflection as there's no public getter)
        $result->citationTargets = self::extractCitationTargets($projectNode);
    ...
    /**
     * Extract citation targets using reflection (no public getter available).
     *
     * @return array<string, CitationTarget>
     */
    private static function extractCitationTargets(ProjectNode $projectNode): array
    {
        try {
            $reflection = new ReflectionClass($projectNode);
            $property = $reflection->getProperty('citationTargets');
            $property->setAccessible(true);
            /** @var array<string, CitationTarget> $targets */
            $targets = $property->getValue($projectNode);
            return $targets;
    ...
    public function mergeIntoProjectNode(ProjectNode $projectNode): void
    {
        ...
        foreach ($this->internalLinkTargets as $linkType => $targets) {
            foreach ($targets as $anchor => $target) {
                ...
                } catch (\phpDocumentor\Guides\Exception\DuplicateLinkAnchorException) {
